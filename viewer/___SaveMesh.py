...
if __name__ == '__main__':
    ...
    # Kugel für den Träger erstellen
    carrier_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.1)
    carrier_sphere.paint_uniform_color([1, 0, 0])  # Rot
    carrier_sphere.compute_vertex_normals()
    vis.add_geometry(carrier_sphere)

    # Liste für Trail-Punkte
    trail_points = []
    trail_line_set = o3d.geometry.LineSet()
    trail_line_set.colors = o3d.utility.Vector3dVector([[1, 0, 0] for _ in range(1000)])  # Rot
    vis.add_geometry(trail_line_set)
    ...
    while enable:
        ...
        # Kugel-Position aktualisieren
        carrier_sphere.translate(np.array(origin) - carrier_sphere.get_center(), relative=True)
        vis.update_geometry(carrier_sphere)

        # Neuen Trailpunkt hinzufügen
        trail_points.append(origin)

        max_trail_length = 100  # z.B. 100 Positionen
        if len(trail_points) > max_trail_length:
            trail_points.pop(0)

        # Punkte und Linien aktualisieren
        if len(trail_points) > 1:
            trail_line_set.points = o3d.utility.Vector3dVector(trail_points)
            lines = [[i, i + 1] for i in range(len(trail_points) - 1)]
            trail_line_set.lines = o3d.utility.Vector2iVector(lines)
            vis.update_geometry(trail_line_set)
