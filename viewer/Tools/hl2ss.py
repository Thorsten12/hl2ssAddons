...
def _create_configuration_for_mrc_video(enable, hologram_composition, recording_indicator, video_stabilization,
    ...
    return struct.pack('<BBBBBBBfffII', 1 if (enable) else 0, 1 if (hologram_composition) else 0,
                       1 if (recording_indicator) else 0, 1 if (video_stabilization) else 0,
                       1 if (blank_protected) else 0, 1 if (show_mesh) else 0, 1 if (shared) else 0, global_opacity,
                       output_width, output_height, video_stabilization_length, hologram_perspective)
...
def _connect_client_rm_depth_ahat(host, port, chunk_size, mode, divisor, profile_z, profile_ab, level, bitrate,
                                  options):
    c = _gatherer()
    c.open(host, port, chunk_size, mode)
...
def _connect_client_pv(host, port, chunk_size, mode, width, height, framerate, divisor, profile, level, bitrate,
                       options):
    c = _gatherer()
    c.open(host, port, chunk_size, mode)
...
def start_subsystem_pv(host, port, enable_mrc, hologram_composition, recording_indicator, video_stabilization,
                       blank_protected, show_mesh, shared, global_opacity, output_width, output_height,
                       video_stabilization_length, hologram_perspective):
    c = _client()
    c.open(host, port)
    ...
    c.sendall(
        _create_configuration_for_mrc_video(enable_mrc, hologram_composition, recording_indicator, video_stabilization,
                                            blank_protected, show_mesh, shared, global_opacity, output_width,
                                            output_height, video_stabilization_length, hologram_perspective))
    c.close()
...
class rx_rm_vlc(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_vlc(self.host, self.port, self.chunk, self.mode, self.divisor, self.profile,
                                              self.level, self.bitrate, self.options)
...
class rx_rm_depth_ahat(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_depth_ahat(self.host, self.port, self.chunk, self.mode, self.divisor,
                                                     self.profile_z, self.profile_ab, self.level, self.bitrate,
                                                     self.options)
...
class rx_rm_depth_longthrow(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_rm_depth_longthrow(self.host, self.port, self.chunk, self.mode, self.divisor,
                                                          self.png_filter)
...
class rx_pv(_context_manager):
    ...
    def open(self):
        self._client = _connect_client_pv(self.host, self.port, self.chunk, self.mode, self.width, self.height,
                                          self.framerate, self.divisor, self.profile, self.level, self.bitrate,
                                          self.options)
...
class rx_extended_audio:
    ...
    def open(self):
        self._client = _connect_client_extended_audio(self.host, self.port, self.chunk, self.mixer_mode,
                                                      self.loopback_gain, self.microphone_gain, self.profile,
                                                      self.level)
...
class _unpack_rm_depth_ahat:
    ...
    def decode(self, payload):
        depth = np.frombuffer(payload, dtype=np.uint16, offset=0, count=Parameters_RM_DEPTH_AHAT.PIXELS).reshape(
            Parameters_RM_DEPTH_AHAT.SHAPE)
        ab = np.frombuffer(payload, dtype=np.uint16, offset=Parameters_RM_DEPTH_AHAT.PIXELS * _SIZEOF.WORD,
...
class _decode_ab_rm_depth_ahat:
    ...
    def decode(self, payload):
        for packet in self._codec.parse(payload):
            for frame in self._codec.decode(packet):
                return np.square(frame.to_ndarray()[:Parameters_RM_DEPTH_AHAT.HEIGHT, :Parameters_RM_DEPTH_AHAT.WIDTH],
                                 dtype=np.uint16)
        return None
...
class _unpack_ab_rm_depth_ahat:
    ...
    def decode(self, payload):
        return np.frombuffer(payload, dtype=np.uint16, offset=0, count=Parameters_RM_DEPTH_AHAT.PIXELS).reshape(
            Parameters_RM_DEPTH_AHAT.SHAPE)
...
class _decode_rm_depth_ahat_zdepth:
    def __init__(self, profile):
        ...
        self._codec_ab = _unpack_ab_rm_depth_ahat() if (profile == VideoProfile.RAW) else _decode_ab_rm_depth_ahat(
            profile)
...
def decode_rm_depth_ahat(profile_z, profile_ab):
    return (_unpack_rm_depth_ahat() if (profile_ab == VideoProfile.RAW) else _decode_rm_depth_ahat(profile_ab)) if (
                profile_z == DepthProfile.SAME) else _decode_rm_depth_ahat_zdepth(profile_ab)
...
def unpack_pv(payload):
    return _PV_Frame(payload[:-16], np.frombuffer(payload[-16:-8], dtype=np.float32),
...
class sm_bounding_volume:
    ...
    def add_box(self, center, extents):
        self._add(struct.pack('<Iffffff', _SM_VolumeType.Box, center[0], center[1], center[2], extents[0], extents[1],
                              extents[2]))

    def add_frustum(self, near, far, right, left, top, bottom):
        self._add(struct.pack('<Iffffffffffffffffffffffff', _SM_VolumeType.Frustum, near[0], near[1], near[2], near[3],
                              far[0], far[1], far[2], far[3], right[0], right[1], right[2], right[3], left[0], left[1],
                              left[2], left[3], top[0], top[1], top[2], top[3], bottom[0], bottom[1], bottom[2],
                              bottom[3]))

    def add_oriented_box(self, center, extents, orientation):
        self._add(struct.pack('<Iffffffffff', _SM_VolumeType.OrientedBox, center[0], center[1], center[2], extents[0],
...
class sm_mesh_task:
    ...
    def add_task(self, id, max_triangles_per_cubic_meter, vertex_position_format, triangle_index_format,
                 vertex_normal_format, include_vertex_normals, include_bounds):
        self._data.extend(
            struct.pack('<16sdIIII', id, max_triangles_per_cubic_meter, vertex_position_format, triangle_index_format,
                        vertex_normal_format, (1 if include_vertex_normals else 0) | (2 if include_bounds else 0)))
        self._count += 1
...
class ipc_sm(_context_manager):
    ...
    def get_observed_surfaces(self):
        ...
        return [_sm_surface_info(ids[(i * 24):(i * 24 + 16)], struct.unpack('<Q', ids[(i * 24 + 16):(i * 24 + 24)])[0])
...
class su_task:
    ...
    def pack(self):
        ...
        self._task.extend(
            struct.pack('<BBBBIfBBBBBBBBI', self.enable_quads, self.enable_meshes, self.enable_only_observed,
                        self.enable_world_mesh, self.mesh_lod, self.query_radius, self.create_mode, self.kind_flags,
                        self.get_orientation, self.get_position, self.get_location_matrix, self.get_quad,
                        self.get_meshes, self.get_collider_meshes, len(self.guid_list)))
