...
class wr_rm_vlc(hl2ss._context_manager):
    ...
    def open(self):
        self._wr = _create_wr_rm_vlc(self.filename, self.port, self.mode, self.divisor, self.profile, self.level,
                                     self.bitrate, self.options, self.user)
...
class wr_rm_depth_ahat(hl2ss._context_manager):
    ...
    def open(self):
        self._wr = _create_wr_rm_depth_ahat(self.filename, self.port, self.mode, self.divisor, self.profile_z,
                                            self.profile_ab, self.level, self.bitrate, self.options, self.user)
...
class wr_rm_depth_longthrow(hl2ss._context_manager):
    ...
    def open(self):
        self._wr = _create_wr_rm_depth_longthrow(self.filename, self.port, self.mode, self.divisor, self.png_filter,
                                                 self.user)
...
class wr_pv(hl2ss._context_manager):
    ...
    def open(self):
        self._wr = _create_wr_pv(self.filename, self.port, self.mode, self.width, self.height, self.framerate,
                                 self.divisor, self.profile, self.level, self.bitrate, self.options, self.user)
...
class wr_extended_audio(hl2ss._context_manager):
    ...
    def open(self):
        self._wr = _create_wr_extended_audio(self.filename, self.port, self.mixer_mode, self.loopback_gain,
                                             self.microphone_gain, self.profile, self.level, self.user)
...
def _create_wr_from_rx_rm_depth_ahat(filename, rx, user):
    return wr_rm_depth_ahat(filename, rx.port, rx.mode, rx.divisor, rx.profile_z, rx.profile_ab, rx.level, rx.bitrate,
                            rx.options, user)
...
def _create_wr_from_rx_pv(filename, rx, user):
    return wr_pv(filename, rx.port, rx.mode, rx.width, rx.height, rx.framerate, rx.divisor, rx.profile, rx.level,
                 rx.bitrate, rx.options, user)
...
def _create_wr_from_rx_extended_audio(filename, rx, user):
    return wr_extended_audio(filename, rx.port, rx.mixer_mode, rx.loopback_gain, rx.microphone_gain, rx.profile,
                             rx.level, user)
...
class _rd_decoded(_rd):
    ...
    __method_table = {
        ...
        __set_codec_rm_depth_ahat, __create_codec_rm_depth_ahat, __decode_rm_depth_ahat),
        hl2ss.StreamPort.RM_DEPTH_LONGTHROW: (
        __set_codec_rm_depth_longthrow, __create_codec_rm_depth_longthrow, __decode_rm_depth_longthrow),
