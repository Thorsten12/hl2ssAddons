...
class integrator:
    def __init__(self, voxel_size=3 / 512, block_resolution=16, block_count=10000, device='cpu:0'):
        ...
        attr_channels = ((1), (1), (3))
        self._vbg = o3d.t.geometry.VoxelBlockGrid(attr_names=attr_names, attr_dtypes=attr_dtypes,
                                                  attr_channels=attr_channels, voxel_size=voxel_size,
                                                  block_resolution=block_resolution, block_count=block_count,
                                                  device=self._device)
        self.set_trunc()
    ...
    def integrate(self):
        frustum_block_coords = self._vbg.compute_unique_block_coordinates(self._depth, self._intrinsics64,
                                                                          self._extrinsics64, self._depth_scale,
                                                                          self._depth_max, self._trunc_voxel_multiplier)
        self._vbg.integrate(frustum_block_coords, self._depth, self._color, self._intrinsics64, self._intrinsics64,
                            self._extrinsics64, self._depth_scale, self._depth_max)
